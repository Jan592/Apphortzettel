// OPTION 1: Hybrid Sidebar - Bottom Nav auf Mobile, Sidebar auf Desktop
...
interface ParentDashboardHybridSidebarProps {
  user: {
    firstName: string;
    lastName: string;
    childProfile?: any;
...
export default function ParentDashboardHybridSidebar({
  user,
  ...
  content = {},
  onToggleDesign
}: ParentDashboardHybridSidebarProps) {
  ...
  // CONTENT RENDERING (same as before, just moved to separate function)
  const renderContent = () => {
    if (currentTab === 'home') {
      return (
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex">
      ...
      <aside className="hidden md:flex md:flex-col md:w-64 lg:w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 fixed h-screen">
        {/* Logo & Branding */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shadow-lg safe-area-inset-bottom z-50">
            ...
            >
              ...
              {item.badgeCount && item.badgeCount > 0 && (
                <div className="absolute top-1 right-2 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center">
                  <span className="text-white text-xs font-medium">{item.badgeCount}</span>
                </div>
              )}
            </button>
          ))}
