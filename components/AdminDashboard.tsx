...
import type { AdminUser, SystemStats, AppSettings, TimeRestrictionSettings, HortzettelData } from "../types/hortzettel";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const [allHortzettel, setAllHortzettel] = useState<(HortzettelData & { id: string; createdAt: string })[]>([]);
  const [hortzettelLoading, setHortzettelLoading] = useState(false);
  const [hortzettelFilter, setHortzettelFilter] = useState<"alle" | "aktiv" | "archiviert">("alle");
  ...
  const loadAllHortzettel = async () => {
    setHortzettelLoading(true);
    try {
      const response = await api.getHortnerHortzettel();
      setAllHortzettel(response.hortzettel);
    } catch (error) {
      console.error("Error loading hortzettel:", error);
      toast.error("Fehler beim Laden der Hortzettel");
    } finally {
      setHortzettelLoading(false);
    }
  };

  const handleDeleteHortzettel = async (hortzettel: HortzettelData & { id: string }) => {
    const confirmMessage = `Möchten Sie den Hortzettel wirklich löschen?\n\nKind: ${hortzettel.childName}\nKlasse: ${hortzettel.class}\nKW ${hortzettel.weekNumber}, ${hortzettel.year}`;
    ...
    try {
      await api.deleteHortzettel(hortzettel.id);
      toast.success(`Hortzettel von ${hortzettel.childName} gelöscht`, { duration: 3000 });
      await loadAllHortzettel();
    } catch (error) {
      console.error("Error deleting hortzettel:", error);
      toast.error("Fehler beim Löschen des Hortzettels");
    }
  };
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md border p-3">
            ...
            <div className="lg:hidden mb-3 text-center">
              ...
              <div className="flex items-center justify-center gap-2">
                {(activeTab === 'overview' || activeTab === 'users' || activeTab === 'messages' || activeTab === 'announcements') && (
            ...
            <TabsList className="hidden lg:flex w-full bg-transparent h-auto p-0 gap-1">
              ...
              <div className="flex flex-nowrap gap-1 flex-1">
                ...
                >
                  ...
                  <span className="font-medium">Mitteilungen an alle</span>
                </TabsTrigger>
              </div>
          ...
          <TabsContent value="data" className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 border">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button
                  onClick={loadAllHortzettel}
                  variant="outline"
                  size="sm"
                  disabled={hortzettelLoading}
                >
                  {hortzettelLoading ? "Lädt..." : "Aktualisieren"}
                </Button>
              </div>
              <p className="text-sm text-muted-foreground mb-4">
                Alle eingegangenen Hortzettel anzeigen und verwalten
              </p>

              {allHortzettel.length === 0 && !hortzettelLoading ? (
                <div className="text-center py-12">
                  <FileText className="h-16 w-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
                  <p className="text-muted-foreground">Keine Hortzettel vorhanden</p>
                  <Button
                    onClick={loadAllHortzettel}
                    variant="outline"
                    className="mt-4"
                ...
                <>
                  <div className="flex gap-2 mb-4">
                    <Button
                      variant={hortzettelFilter === "alle" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setHortzettelFilter("alle")}
                    >
                      Alle ({allHortzettel.length})
                    </Button>
                    <Button
                      variant={hortzettelFilter === "aktiv" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setHortzettelFilter("aktiv")}
                    >
                      Aktiv ({allHortzettel.filter(h => h.status === 'aktiv').length})
                    </Button>
                    <Button
                      variant={hortzettelFilter === "archiviert" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setHortzettelFilter("archiviert")}
                    >
                      Archiviert ({allHortzettel.filter(h => h.status === 'archiviert').length})
                    </Button>
                  </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b">
                          <th className="text-left py-3 px-4 font-semibold text-sm">Kind</th>
                          <th className="text-left py-3 px-4 font-semibold text-sm">Klasse</th>
                          <th className="text-left py-3 px-4 font-semibold text-sm">Woche</th>
                          <th className="text-left py-3 px-4 font-semibold text-sm">Status</th>
                          <th className="text-left py-3 px-4 font-semibold text-sm">Erstellt</th>
                          <th className="text-right py-3 px-4 font-semibold text-sm">Aktionen</th>
                        </tr>
                      </thead>
                      <tbody>
                        {allHortzettel
                          .filter(h => hortzettelFilter === "alle" || h.status === hortzettelFilter)
                          .map((hortzettel) => (
                            <tr key={hortzettel.id} className="border-b hover:bg-slate-50 dark:hover:bg-slate-700/30">
                              <td className="py-3 px-4">
                                <div className="font-medium">{hortzettel.childName}</div>
                              </td>
                              <td className="py-3 px-4">{hortzettel.class}</td>
                              <td className="py-3 px-4">KW {hortzettel.weekNumber}, {hortzettel.year}</td>
                              <td className="py-3 px-4">
                                <Badge variant={hortzettel.status === 'aktiv' ? 'default' : 'secondary'}>
                                  {hortzettel.status}
                                </Badge>
                              </td>
                              <td className="py-3 px-4 text-sm text-muted-foreground">
                                {new Date(hortzettel.createdAt).toLocaleDateString('de-DE')}
                              </td>
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => handleDeleteHortzettel(hortzettel)}
                                  ...
                                  <TooltipContent>
                                    <p className="text-xs">Hortzettel löschen</p>
                                  </TooltipContent>
                                </Tooltip>
                              </td>
                            </tr>
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Clock className="h-5 w-5 text-orange-600" />
              Zeitsperren & Automatisches Archivieren
            </DialogTitle>
            <DialogDescription>
              Legen Sie fest, wann Eltern Hortzettel erstellen und bearbeiten dürfen und wann die Woche automatisch abgeschlossen wird.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 pt-4">
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
              <h4 className="text-sm font-semibold mb-4 text-slate-700 dark:text-slate-300">Wochenwechsel & Archivierung</h4>
              <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-lg border">
                  <div className="flex-1 pr-4">
                    <Label className="text-base font-medium">Automatische Archivierung</Label>
                    <p className="text-sm text-muted-foreground mt-1">
                      Wenn aktiviert, werden alle aktiven Zettel der letzten Woche zum gewählten Zeitpunkt ins Archiv verschoben.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                    checked={timeRestrictions.autoArchiveEnabled}
                    onChange={(e) => 
                      setTimeRestrictions({ ...timeRestrictions, autoArchiveEnabled: e.target.checked })
                    }
                    className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                </div>

                {timeRestrictions.autoArchiveEnabled && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border rounded-lg">
                    <div>
                      <Label className="text-base font-medium">Wochentag</Label>
                      <p className="text-sm text-muted-foreground mb-3">An welchem Tag soll archiviert werden?</p>
                      <select 
                        className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm"
                        value={timeRestrictions.autoArchiveDay}
                        onChange={(e) => setTimeRestrictions({...timeRestrictions, autoArchiveDay: parseInt(e.target.value)})}
                      >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mb-3">Um wie viel Uhr?</p>
                      <div className="flex items-center gap-2">
                        <input
                          type="range"
                          min="0"
                          max="23"
                          value={timeRestrictions.autoArchiveHour}
                          onChange={(e) => 
                            setTimeRestrictions({ 
                              ...timeRestrictions, 
                              autoArchiveHour: parseInt(e.target.value) || 0 
                            })
                          }
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                        />
                        <span className="text-sm font-medium w-16 text-right">
                          {timeRestrictions.autoArchiveHour.toString().padStart(2, '0')}:00
                        </span>
                      </div>
                    </div>
                  </div>
