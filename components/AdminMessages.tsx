...
export default function AdminMessages() {
  ...
  const handleSendNewMessage = async () => {
    if (!newMessage.receiverId || !newMessage.subject || !newMessage.message) {
    ...
    try {
      ...
      await api.adminSendDirectMessage(
      ...
      setNewMessage({ receiverId: '', receiverName: '', subject: '', message: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleOpenNewMessage} className="gap-2 shrink-0">
      ...
      <Dialog open={showNewMessageDialog} onOpenChange={setShowNewMessageDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Neue Nachricht an Benutzer senden</DialogTitle>
            <DialogDescription>
              Wählen Sie einen Elternteil oder Hortner aus, um eine direkte Nachricht zu senden.
            </DialogDescription>
          </DialogHeader>
              ...
              <select 
                ...
                onChange={(e) => {
                  ...
                  setNewMessage({
                    ...
                    receiverName: selectedUser ? `${selectedUser.firstName} ${selectedUser.lastName}` : ''
                  });
                }}
                disabled={loadingUsers}
              >
                <option value="">-- Empfänger auswählen --</option>
                {users.filter(u => u.role === 'parent' || u.role === 'hortner').map(user => (
                  <option key={user.id} value={user.id}>
                    {user.firstName} {user.lastName} ({user.role === 'hortner' ? 'Hortner' : 'Elternteil'})
                  </option>
                ))}
              </select>
            </div>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={sendingNewMessage || !newMessage.receiverId || !newMessage.subject || !newMessage.message}
            >
              {sendingNewMessage ? 'Wird gesendet...' : 'Nachricht senden'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
