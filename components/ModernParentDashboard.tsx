...
export default function ModernParentDashboard({
  ...
  const handleUseTemplateAsHortzettel = async (template: any) => {
    try {
      ...
      // Spezielle Behandlung für "bereits vorhanden" Fehler
      if (error.status === 409 || error.message?.includes('bereits ein Hortzettel')) {
        const details = error.data?.details || 'Für diese Woche existiert bereits ein Hortzettel.';
        const weekInfo = error.data?.weekNumber ? ` (KW ${error.data.weekNumber})` : '';
        
        toast.error(`Hortzettel bereits vorhanden${weekInfo}`, {
          duration: 7000,
          description: details
        });
      } else {
