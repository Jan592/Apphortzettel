...
export class ApiClient {
  ...
  async adminSendDirectMessage(receiverId: string, receiverName: string, subject: string, message: string) {
    return this.request<{
      success: boolean;
      message: any;
    }>('/admin/messages/send', {
      method: 'POST',
      body: JSON.stringify({ receiverId, receiverName, subject, message }),
    });
  }
